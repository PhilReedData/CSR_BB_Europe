...
# Considers two limits (first N characters of file).
# Three choices:
# findModeYearInTXTLim1then2 - get year in first limit, else second.
# findModeYearInTXTLim1and2 - get tuple year in first and second limits.
# findModeYearInTXTLim2only - get year in second limit only.
# Year is 2 if not found, or -3 if value error.
# Recommend using findModeYearInTXTLim2only
...
def getYearStringValue(ys):
    try:
        ...
        year = -3
    year = end if year > end else year # don't go past the end of the range.
    return year
...
def findModeYearInTXTLim1then2(path):
    text = u''
...
def findModeYearsInTXTLim1and2(path):
    text = u''
    print ('Opening', path)
    with codecs.open(path, 'r', encoding='utf8', errors='ignore') as fileIn:
        # Read in file as far as LIMIT_2
...
# Search given text for most occuring year, or -2 if none.
# Look at first <LIMIT_2> characters only.
# If no year found, give up.
def findModeYearInTXTLim2only(path):
    text = u''
    with codecs.open(path, 'r', encoding='utf8', errors='ignore') as fileIn:
        # Read in file as far as LIMIT_2
        text = fileIn.read(LIMIT_2)
...
if __name__ == "__main__":
    testString = "2001 2006 2009 200120092008"
    y = findModeYearInTXTHead(testString, len(testString))
    print('mode',y)
